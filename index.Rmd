...
### Plot 4: Cumulative deaths (days since 10th death)
...
## Brazil
deaths_brazil <- deaths_jhu %>% 
  filter(country == 'Brazil') %>% 
  select(-`Province/State`)  %>% 
  pivot_longer(cols = 2:(ncol(deaths_jhu)-1),
               names_to = 'date',
               values_to = 'brazil_count') %>% 
  filter(brazil_count > 0) %>% 
  mutate(days = row_number()) %>% 
  select(-date) %>% 
  select(-country)

## France
deaths_france <- deaths_jhu %>% 
  filter(country == 'France') %>% 
  filter(is.na(`Province/State`)) %>% 
  select(-`Province/State`) %>% 
  pivot_longer(cols = 2:(ncol(deaths_jhu)-1),
...
## Italy
deaths_italy <- deaths_jhu %>% 
  filter(country == 'Italy') %>% 
  select(-`Province/State`)  %>% 
  pivot_longer(cols = 2:(ncol(deaths_jhu)-1),
               names_to = 'date',
               values_to = 'italy_count')%>% 
  filter(italy_count > 0) %>% 
  mutate(days = row_number()) %>% 
  select(-date) %>% 
  select(-country)
...
## Sweden
deaths_sweden <- deaths_jhu %>% 
  filter(country == 'Sweden') %>% 
  select(-`Province/State`)  %>% 
  pivot_longer(cols = 2:(ncol(deaths_jhu)-1),
               names_to = 'date',
               values_to = 'sweden_count') %>% 
  filter(sweden_count > 0) %>% 
  mutate(days = row_number()) %>% 
  select(-date) %>% 
  select(-country)

## Denmark
deaths_denmark <- deaths_jhu %>% 
  filter(country == 'Denmark' & is.na(`Province/State`)) %>% 
  select(-`Province/State`)  %>% 
  pivot_longer(cols = 2:(ncol(deaths_jhu)-1),
               names_to = 'date',
               values_to = 'denmark_count') %>% 
  filter(denmark_count > 0) %>% 
  mutate(days = row_number()) %>% 
  select(-date) %>% 
  select(-country)

# Bind columns
deaths_jhu2 <- deaths_china %>% 
  left_join(deaths_italy) %>% 
  left_join(deaths_us) %>% 
  left_join(deaths_brazil) %>% 
  left_join(deaths_denmark) %>% 
  left_join(deaths_france) %>% 
  left_join(deaths_nz) %>% 
  left_join(deaths_uk) %>% 
  left_join(deaths_australia) %>% 
  left_join(deaths_ned) %>% 
  left_join(deaths_za) %>% 
  left_join(deaths_sweden)
...
plot_ly(data = deaths_jhu2,
        ...
        marker = list(size = 0),
        mode = 'lines+markers',
        hovertemplate = paste("<b>Days since 1st death: </b>%{x}<br><b>Deaths: <b>%{y}")) %>% 
    layout(font = list(family = 'Arial'),
           xaxis = list(title = 'Days since 1st death',
                        titlefont = list(size = 20)),
           yaxis = list(type = 'log',
...
### Plot 6: Doubling time for cases (in days) for each week
...
```{r eval = FALSE}
### Percentage of daily tests that are positive for SARS-CoV-2
